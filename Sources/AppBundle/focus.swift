...
/// But, in general, commands must firstly check --window-id, --workspace, AXIS_WINDOW_ID env and
/// AXIS_WORKSPACE env before accessing the global focus.
...
@MainActor private func onWorkspaceChanged(_ oldWorkspace: String, _ newWorkspace: String) {
    ...
    if let exec = config.execOnWorkspaceChange.first {
        ...
        environment["AXIS_FOCUSED_WORKSPACE"] = newWorkspace
        environment["AXIS_PREV_WORKSPACE"] = oldWorkspace
        environment[AXIS_WORKSPACE] = newWorkspace
