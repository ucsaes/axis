...
public func getMessageWindow(messageModel: MessageModel) -> some Scene {
    ...
    SwiftUI.Window(messageModel.message?.title ?? axisAppName, id: messageWindowId) {
...
public let messageWindowId = "\(axisAppName).messageView"
...
public struct Message: Hashable, Equatable {
    ...
    init(type: MessageType = .config, title: String = axisAppName, description: String, body: String) {
        self.type = type
        self.title = title
